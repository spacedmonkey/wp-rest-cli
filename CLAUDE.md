...
The *top-level* bare listings are a separate case from the above (they list child items, not a single route/verb's own args) and aren't gated behind `--help` at all: bare `wp`/`wp help` (namespaces) and bare `wp <namespace>`/`wp help <namespace>` (routes) render a WP-CLI-native NAME/DESCRIPTION/SYNOPSIS/SUBCOMMANDS page by default in `--format=table`, via `renderChildListWpCli` — mirroring what real WP-CLI's own bare `wp` prints, with each subcommand's "description" being its supported verbs (there's no real per-namespace/per-route description to show). Any other `--format`, and a route's own nested-children note (`renderChildrenNote`, appended after a "hybrid" route's regular schema output — see `routeChildren` above), keep returning the plain `{route, verbs}` rows (`buildChildRows`/`renderRouteChildren`) instead, since those are either consumed by scripts or nested inside another block where a full NAME/DESCRIPTION/SYNOPSIS page would be out of place.
