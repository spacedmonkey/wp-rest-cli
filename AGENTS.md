...
wp-rest-cli wp/v2                          # routes in a namespace (with their verbs)
wp-rest-cli wp/v2 posts                    # the route's schema: args, types, enums
wp-rest-cli help wp/v2 posts create        # one verb's schema; add --format=json for structured output
```
...
- `exists <id>` exits `1` for "not found". An unknown namespace or route exits `1` with `No such namespace`/`No such route`.
