...
- If the test touches anything persisted via `src/config.ts` (`wp auth`/`wp config` state), don't use the top-level `run()` helper — it doesn't isolate the config store, so tests would read/clobber whatever real `wp-rest-cli` config exists on the machine running them. Follow the `describe('auth', ...)` block's own `runAuth(args)` pattern instead: a `beforeAll`/`afterAll`-scoped temp directory passed as `XDG_CONFIG_HOME` in `execa`'s `env`, isolating that block's config store per test run.
