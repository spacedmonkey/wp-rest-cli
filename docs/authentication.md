...
- **[OAuth2](authentication-oauth2.md)** (`oauth2`) — via the [WP-API/OAuth2](https://github.com/WP-API/OAuth2) plugin, for a site that already runs it. Supports the browser-based `authorization_code` grant, the no-browser `client_credentials` grant, and a personal access token, generated by hand in wp-admin.
