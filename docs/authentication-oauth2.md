...
## `wp auth oauth2 add` — no browser, client credentials (`client_credentials`)
...
## `wp auth oauth2 add` — no browser, personal access token

```sh
wp-rest-cli auth oauth2 add https://example.com --token=<your-personal-token>
```
...
```sh
$ wp-rest-cli auth oauth2 add https://example.com --token=wrong
...
```sh
wp-rest-cli auth oauth2 add https://example.com --token=<your-personal-token> skip-verify=true
```
...
```sh
wp-rest-cli auth oauth2 list      # every stored site's client id (blank for a personal token) and grant type, never the access token
...
- **No self-service client registration** — see the prerequisite above. (Doesn't apply to a personal access token — see [personal access token](#wp-auth-oauth2-add--no-browser-personal-access-token) above — since there's no client/Application involved in that path at all.)
