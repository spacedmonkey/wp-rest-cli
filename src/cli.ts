...
const FORMATS: OutputFormat[] = ['table', 'json', 'csv', 'yaml', 'ids', 'count', 'raw'];
...
function normalizeDynamicFlags(argv: string[]): string[] {
  return argv.map((token) => {
    ...
    if (withValue) {
      ...
      return KNOWN_LONG_FLAGS.has(name as string) ? token : `${name}=${value}`;
    }
    const boolFlag = token.match(/^--([a-zA-Z0-9_-]+)$/);
    if (boolFlag) {
      const [, name] = boolFlag;
      return KNOWN_LONG_FLAGS.has(name as string) ? token : `${name}=true`;
    }
    return token;
  });
}
...
function toGlobalFlags(options: RawOptions): GlobalFlags {
  if (!CONTEXTS.includes(options.context as Context)) {
    throw new CliError(
      `--context must be one of: ${CONTEXTS.join(', ')} (got "${options.context}").`,
    );
  }
  if (!FORMATS.includes(options.format as OutputFormat)) {
    throw new CliError(`--format must be one of: ${FORMATS.join(', ')} (got "${options.format}").`);
  }
  return {
    url: options.url,
...
async function handleConfigCommand(args: string[], options: RawOptions): Promise<number> {
  const [, sub] = args;
  switch (sub) {
    case 'get': {
    ...
    case 'set': {
      if (!options.url && !options.username) {
        console.error(
          formatErrorForDisplay(new CliError('config set requires --url and/or --username.')),
        );
        return 1;
      }
    ...
    default: {
      console.error(
        formatErrorForDisplay(
          new CliError('Usage: wp config <get|set|clear> [--url=] [--username=]'),
        ),
      );
      return 1;
    }
...
async function handleHelpCommand(args: string[], options: RawOptions): Promise<number> {
  if (args.length === 0) {
    program.outputHelp();
    return 0;
  }
...
program
  ...
  .option('--username <username>', 'Username or email (also WP_USERNAME env var)')
  .option('--password <password>', 'Password or Application Password (also WP_PASSWORD env var)')
  ...
  .option('--content <content>', 'Raw JSON body for create/update, overriding field=value args')
  .option('--no-color', 'Disable colored output')
  .option('--quiet', 'Suppress spinner/progress output')
  .option('--debug', 'Log each HTTP request/response (to stderr) and show extra output on error')
  .addHelpText(
    'after',
    `
Examples:
...
`,
  ...
  .action(async (args: string[], options: RawOptions) => {
    try {
      ...
      if (args[0] === 'help') {
        process.exitCode = await handleHelpCommand(args.slice(1), options);
        return;
      }
      ...
      const { output, exitCode } = await runRestCommand(parsed, flags, siteUrl);
      console.log(output);
