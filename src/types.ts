 ...
 * `buildAuth`'s normal `WP_USERNAME`/`WP_PASSWORD` env vars → stored `wrapido auth`
 ...
 *
 * A non-`env`/`none` value names a `wrapido auth` type (`application-passwords` or
...
export interface GlobalFlags {
	...
	username?: string;
	password?: string;
	/** OAuth2 client id, for `wrapido auth oauth2 login`/`add` — a global flag (like `username`/`password` above) rather than a subcommand-scoped field token, since it's a real credential value. */
	clientId?: string;
	/** OAuth2 client secret, for `wrapido auth oauth2 login`/`add`. */
	clientSecret?: string;
	/** OAuth2 personal access token, generated by hand in wp-admin, for `wrapido auth oauth2 add` — an alternative to `clientId`/`clientSecret`'s `client_credentials` grant. */
	token?: string;
	useAuth?: AuthSource;
	context: Context;
	format: OutputFormat;
