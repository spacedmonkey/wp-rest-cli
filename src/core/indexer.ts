...
export async function fetchIndex(client: WpRestClient, apiRoot: string): Promise<IndexResponse> {
...
export function routesForNamespace(
  ...
  for (const path of paths) {
    if (path === `/${namespace}`) continue; // the namespace root itself
    ...
    if (path.includes('(?P<')) {
      ...
      if (basePath === path || paths.includes(basePath)) continue;
...
const VERB_ORDER: Verb[] = ['list', 'get', 'create', 'update', 'delete', 'exists', 'generate'];
 ...
 * `exists`, not `list`.
 */
export function supportedVerbsForRoute(index: IndexResponse, path: string): Verb[] {
  ...
  const collectionMethods = new Set(isItemOnlyPath ? [] : (index.routes[path]?.methods ?? []));
  const itemMethods = new Set(itemPath ? (index.routes[itemPath]?.methods ?? []) : []);

  const supported = new Set<Verb>();
  if (collectionMethods.has('GET')) supported.add('list');
  ...
  if (itemMethods.has('PUT') || itemMethods.has('PATCH')) supported.add('update');
  if (itemMethods.has('DELETE')) supported.add('delete');
...
export function resolveRouteInfo(
  ...
  if (index.routes[exactPath]) return { path: exactPath, requiresParam: false };
  for (const path of Object.keys(index.routes)) {
    if (!path.includes('(?P<')) continue;
    const basePath = stripTrailingPlaceholder(path);
    if (basePath !== path && basePath === exactPath) return { path, requiresParam: true };
  }
  return { path: exactPath, requiresParam: false };
}

export function isApplicationPasswordsSupported(index: IndexResponse): boolean {
  return Boolean(index.authentication?.['application-passwords']?.endpoints?.authorization);
}
