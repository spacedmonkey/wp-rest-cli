...
describe('formatErrorForDisplay', () => {
  it('formats a WpApiError with its code and status', () => {
    const error = new WpApiError({ code: 'rest_forbidden', message: 'Not allowed.' }, 401);
    expect(formatErrorForDisplay(error)).toBe('Error: Not allowed. (rest_forbidden, status 401)');
  });

  it('formats a CliError with just its message', () => {
    expect(formatErrorForDisplay(new CliError('Missing --url.'))).toBe('Error: Missing --url.');
  });
});
