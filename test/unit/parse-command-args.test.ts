...
describe('parseCommandArgs', () => {
  ...
  it('parses a single arg as the routes mode', () => {
    expect(parseCommandArgs(['wp/v2'])).toEqual({ mode: 'routes', namespace: 'wp/v2' });
  });
  ...
  it('parses list with field=value args', () => {
    expect(parseCommandArgs(['wp/v2', 'posts', 'list', 'per_page=5'])).toEqual({
      mode: 'verb',
      namespace: 'wp/v2',
      route: 'posts',
      verb: 'list',
  ...
  it('throws when get is missing its id', () => {
    expect(() => parseCommandArgs(['wp/v2', 'posts', 'get'])).toThrow(CliError);
  });

  it('throws when get is given a field=value token instead of an id', () => {
    expect(() => parseCommandArgs(['wp/v2', 'posts', 'get', 'title=Hello'])).toThrow(CliError);
  });

  it('parses update with an id and field=value args', () => {
    expect(parseCommandArgs(['wp/v2', 'posts', 'update', '42', 'title=Hello'])).toEqual({
      mode: 'verb',
      namespace: 'wp/v2',
      route: 'posts',
      verb: 'update',
  ...
  it('throws when a field=value token appears before any verb', () => {
    expect(() => parseCommandArgs(['wp/v2', 'posts', 'title=Hello'])).toThrow(CliError);
  });

  it('joins several route segments given as separate args, stopping at the verb', () => {
    expect(
      parseCommandArgs(['wp/v2', 'global-styles', 'themes', 'get', 'twentytwentyfour']),
    ).toEqual({
      mode: 'verb',
      namespace: 'wp/v2',
  ...
  it('joins route segments with no verb into a single introspect route', () => {
    expect(parseCommandArgs(['wp/v2', 'global-styles', 'themes'])).toEqual({
      mode: 'introspect',
      namespace: 'wp/v2',
      route: 'global-styles/themes',
    });
  ...
  it('splits field=value only on the first "="', () => {
    expect(parseCommandArgs(['wp/v2', 'posts', 'create', 'content=a=b'])).toEqual({
      mode: 'verb',
      namespace: 'wp/v2',
      route: 'posts',
      verb: 'create',
