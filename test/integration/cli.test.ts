...
function run(args: string[]) {
  return execa('tsx', [cliEntry, ...args, `--url=${fixture.baseUrl}`, '--quiet', '--no-color'], {
    reject: false,
    preferLocal: true,
  });
}
...
describe('wp-rest-cli (integration)', () => {
  ...
  it('lists routes for a namespace, with a verbs column instead of raw HTTP methods', async () => {
    ...
    expect(result.exitCode).toBe(0);
    const rows = JSON.parse(result.stdout) as Array<{ route: string; verbs: string }>;
    const widgets = rows.find((r) => r.route === 'widgets');
    expect(widgets?.verbs).toBe('list, get, create, update, delete, exists, generate');
  });
  ...
  it('shows a WP-CLI-style usage synopsis covering every verb', async () => {
    ...
    expect(lines[1]).toBe('   or: wp-rest-cli wp/v2 widgets get <id> [--context=<context>]');
    expect(lines[2]).toBe(
    ...
    expect(lines[4]).toBe('   or: wp-rest-cli wp/v2 widgets delete <id> [--force]');
  ...
  it('lists items for a route', async () => {
    ...
    expect(body).toEqual([
      { id: 1, title: { rendered: 'First widget' }, link: '/widgets/1', meta: {} },
    ]);
  });

  it('gets a single item by id', async () => {
    const result = await run(['wp/v2', 'widgets', 'get', '1', '--field=title.rendered']);
    expect(result.exitCode).toBe(0);
  ...
  it('creates an item from field=value args', async () => {
    const result = await run(['wp/v2', 'widgets', 'create', '--title=New widget']);
    expect(result.exitCode).toBe(0);
  ...
  it('shows list-verb help with the matching collection GET args', async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('usage: wp-rest-cli wp/v2 widgets list');
  ...
  it('shows delete-verb help without performing the delete', async () => {
    ...
    const stillThere = await run(['wp/v2', 'widgets', 'get', '2', '--field=id']);
  ...
  it('folds an unrecognised verb into the route for `help`, surfacing the live 404 instead of a local error', async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain('No route was found matching the URL and request method.');
  });
  ...
  it('performs a get against a route addressed as separate CLI arguments', async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(JSON.parse(result.stdout)).toEqual({ settings: {}, styles: {} });
  });
  ...
  describe('singleton routes (e.g. settings)', () => {
    it('does not offer get/update/delete/exists <id> for a route with no addressable id', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('usage: wp-rest-cli wp/v2 settings list');
      ...
      expect(result.stdout).toContain("This route has no addressable <id>");
    });
    ...
    it('reads and writes the singleton via list/create, not get/update', async () => {
      const before = await run(['wp/v2', 'settings', 'list', '--format=json']);
      expect(before.exitCode).toBe(0);
      expect(JSON.parse(before.stdout)).toEqual({ title: 'Fixture Site' });
      ...
      expect(JSON.parse(updated.stdout)).toEqual({ title: 'Updated Site' });
    });
  });
  ...
  describe('exists', () => {
    it('exits 0 and reports success for an id that exists', async () => {
      ...
      const result = await run(['wp/v2', 'widgets', 'exists', String(id)]);
      expect(result.exitCode).toBe(0);
    ...
    it('reports a structured boolean under --format=json', async () => {
      const result = await run(['wp/v2', 'widgets', 'exists', '999999', '--format=json']);
      expect(result.exitCode).toBe(1);
  ...
  describe('generate', () => {
    it('creates --count items reusing the same fields for each', async () => {
      const before = await run(['wp/v2', 'widgets', 'list', '--format=count']);
      ...
      const after = await run(['wp/v2', 'widgets', 'list', '--format=json']);
      const items = JSON.parse(after.stdout) as Array<{ title: { rendered: string } }>;
      expect(items.length).toBe(beforeCount + 3);
      expect(items.filter((w) => w.title.rendered === 'Bulk widget').length).toBe(3);
    });

    it('rejects a non-positive-integer --count', async () => {
      const result = await run(['wp/v2', 'widgets', 'generate', '--count=0']);
      expect(result.exitCode).toBe(1);
  ...
  describe('--debug', () => {
    it('logs each HTTP request/response to stderr, leaving stdout as clean formatted output', async () => {
      const result = await run(['wp/v2', 'widgets', 'list', '--debug', '--format=json']);
      expect(result.exitCode).toBe(0);
      expect(result.stderr).toContain(`GET ${fixture.baseUrl}/wp-json/wp/v2/widgets`);
  ...
  describe('meta', () => {
    async function createWidget(title: string): Promise<string> {
      const result = await run(['wp/v2', 'widgets', 'create', `--title=${title}`, '--format=json']);
    ...
    it('shows the meta usage synopsis for a route that supports meta', async () => {
      const result = await run(['wp/v2', 'widgets']);
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('usage: wp-rest-cli wp/v2 widgets meta add <id> <key>');
      expect(result.stdout).toContain('wp-rest-cli wp/v2 widgets meta list <id>');
    });

    it('sets and reads back a meta value with update/get', async () => {
      const id = await createWidget('Meta target');
      const updateResult = await run(['wp/v2', 'widgets', 'meta', 'update', id, 'color', 'blue']);
      expect(updateResult.exitCode).toBe(0);
      expect(updateResult.stdout).toContain('Success');

      const getResult = await run(['wp/v2', 'widgets', 'meta', 'get', id, 'color']);
    ...
    it('add behaves the same as update (no separate multi-value semantics over REST)', async () => {
      ...
      await run(['wp/v2', 'widgets', 'meta', 'add', id, 'color', 'green']);
      const getResult = await run(['wp/v2', 'widgets', 'meta', 'get', id, 'color']);
    ...
    it('lists every meta key as key/value rows', async () => {
      ...
      await run(['wp/v2', 'widgets', 'meta', 'update', id, 'color', 'red']);
      const result = await run(['wp/v2', 'widgets', 'meta', 'list', id, '--format=json']);
      expect(result.exitCode).toBe(0);
      expect(JSON.parse(result.stdout)).toEqual([{ key: 'color', value: 'red' }]);
    });

    it('deletes a whole meta key', async () => {
      const id = await createWidget('Meta delete target');
      await run(['wp/v2', 'widgets', 'meta', 'update', id, 'color', 'yellow']);
      const del = await run(['wp/v2', 'widgets', 'meta', 'delete', id, 'color']);
    ...
    it('removes every matching value from an array-type meta field via delete <key> <value>', async () => {
      ...
      await run(['wp/v2', 'widgets', 'meta', 'update', id, 'tags', '["a","b","a"]']);
      await run(['wp/v2', 'widgets', 'meta', 'delete', id, 'tags', '"a"']);
      const getResult = await run(['wp/v2', 'widgets', 'meta', 'get', id, 'tags', '--format=json']);
    ...
    it('dedupes an array-type meta field with clean-duplicates', async () => {
      ...
      await run(['wp/v2', 'widgets', 'meta', 'update', id, 'tags', '["a","b","a","b"]']);
      const clean = await run(['wp/v2', 'widgets', 'meta', 'clean-duplicates', id, 'tags']);
    ...
    it('reads a nested value with pluck and writes one with patch', async () => {
      ...
      await run(['wp/v2', 'widgets', 'meta', 'update', id, 'color', '{"shade":"dark"}']);
      const pluck = await run(['wp/v2', 'widgets', 'meta', 'pluck', id, 'color', 'shade']);
      expect(pluck.stdout.trim()).toBe('dark');

      await run(['wp/v2', 'widgets', 'meta', 'patch', 'update', id, 'color', 'shade', 'light']);
      const after = await run(['wp/v2', 'widgets', 'meta', 'pluck', id, 'color', 'shade']);
