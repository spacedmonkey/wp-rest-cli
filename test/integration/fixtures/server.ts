...
const widgets = new Map<number, Record<string, unknown>>([
  [1, { id: 1, title: { rendered: 'First widget' }, link: '/widgets/1', meta: {} }],
]);
let nextId = 2;
...
export async function startFixture(): Promise<Fixture> {
  const server = createServer(async (req, res) => {
    ...
    if (req.method === 'HEAD' && path === '/') {
      res.writeHead(200, { link: `<${baseUrlHolder.value}/wp-json/>; rel="https://api.w.org/"` });
      res.end();
      return;
    }
      ...
      send(res, 200, {
        ...
        routes: {
          ...
          '/wp/v2/widgets/(?P<id>[\\d]+)': {
            ...
            endpoints: [{ methods: ['GET'] }, { methods: ['PUT'] }, { methods: ['DELETE'] }],
          },
          ...
          '/wp/v2/settings': {
            ...
            endpoints: [{ methods: ['GET', 'POST', 'PUT', 'PATCH'] }],
          },
        },
      });
      return;
      ...
      send(res, 200, {
        ...
        endpoints: [
          ...
          },
          {
            methods: ['POST'],
            args: {
              title: { type: 'string', required: true, description: 'The widget title.' },
              meta: {
                type: 'object',
                description: 'Meta fields.',
                properties: {
                  color: { type: 'string', description: 'Accent color.' },
                  tags: { type: 'array', description: 'Multi-value tag list.' },
                },
              },
            },
          },
    ...
    if (path === '/wp-json/wp/v2/widgets' && req.method === 'POST') {
      ...
      const record = { id, title: { rendered: String(body.title ?? '') }, meta };
      widgets.set(id, record);
      send(res, 201, record);
      return;
    }

    const singularMatch = path.match(/^\/wp-json\/wp\/v2\/widgets\/(\d+)$/);
    if (singularMatch) {
      ...
      if (req.method === 'PUT') {
        ...
        const updated = {
          ...existing,
          title: body.title !== undefined ? { rendered: String(body.title) } : existing.title,
          meta,
        };
    ...
    if (path === '/wp-json/wp/v2/settings' && (req.method === 'POST' || req.method === 'PUT')) {
      const body = await readBody(req);
    ...
    const themeStylesMatch = path.match(/^\/wp-json\/wp\/v2\/global-styles\/themes\/([^/]+)$/);
    if (themeStylesMatch && req.method === 'GET') {
      const stylesheet = decodeURIComponent(themeStylesMatch[1] as string);
